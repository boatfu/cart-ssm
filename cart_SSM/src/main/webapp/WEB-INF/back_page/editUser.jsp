    ...
    <div class="container h-100">
        <div class="row h-100 align-items-center justify-content-center text-center">
            ...
            <div class="col-xs-6 col-md-4 col-center-block">
                <form class="form-signin" action="/editUser" method="post">
                    ${msg}
                    <label for="username" class="sr-only">用户名</label>
                    <input type="text" id="username" name="name" class="form-control" placeholder="用户名:${name}" required=""
                           autofocus="" value="${name}">
                    <label for="inputPassword" class="sr-only">密码</label>
                    <input type="text" id="inputPassword" name="password" class="form-control" placeholder="密码:${password}"
                           required="" value="${password}"><label for="email" class="sr-only">电子邮箱</label><input type="text" id="email"
                                                                                             name="email"
                                                                                             class="form-control"
                                                                                             placeholder="电子邮箱:${email}"
                                                                                             required="" value="${email}">
                    <button class="btn btn-lg btn-primary btn-block" type="submit" name="id" value="${id}">修改</button>
                </form>
            </div>
